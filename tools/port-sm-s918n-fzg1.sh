...
# Root My Galaxy — SM-S918N port/prepare script
# Faithful copy of tools/port-sm-s918b-afzf5.sh, adapted to the S918NKSS8FZG1
# (SM-S918N / dm3q) profile and to the assets actually bundled in this fork.

PROFILE_ID="dm3q-S918NKSS8FZG1-ksunext"
TARGET_MODEL="SM-S918N"
TARGET_DEVICE="dm3q"
# Confirm with: adb shell getprop ro.build.display.id
TARGET_BUILD_DISPLAY="BP4A.251205.006.S918NKSS8FZG1"
# Confirm with: adb shell getprop ro.build.fingerprint
TARGET_FINGERPRINT="samsung/dm3qxxx/dm3q:16/BP4A.251205.006/S918NKSS8FZG1:user/release-keys"
# Confirmed from the S918N kernel image: strings kernel | grep 'Linux version'
TARGET_KERNEL_RELEASE="5.15.189-android13-8-33413713-abS918NKSS8FZG1"
# Fill from: adb shell uname -v  (empty => the strict kernel-version check is skipped)
TARGET_KERNEL_VERSION=""
TARGET_SDK="36"
TARGET_ABI="arm64-v8a"
TARGET_PAGE_SIZE="4096"

# --- MD5/size expectations for the S918N assets actually bundled in the repo ---
EXPECTED_BASE_MD5="3c82d4f678bd58846facf3e4ad356a33"   # F731U upstream base (rmg-f731u); update if your S918N lineage base differs
EXPECTED_PAYLOAD_MD5="22c39e76f57e99b25e811d5a3226ffb1" # patched/bundled S918N payload
EXPECTED_PAYLOAD_SIZE="131072"
EXPECTED_KSUD_MD5="bd9080bc728f3b98f0239236cd2e22ec"    # ksud-f731u-kdp (device-tested, generic android13-5.15 KMI)
EXPECTED_KSUD_SIZE="6756208"
EXPECTED_HELPER_MD5="2451b6aa907b611740c12d8b2e0630ed"   # libcve43499root.so (Regular)
EXPECTED_HELPER_NEXT_MD5="ad8570ed405a7aff92bb340b924b6c3a" # libcve43499root_next.so (Next)
...
SPEC="${SPEC:-${REPO_ROOT}/tools/f731u-to-dm3q-s918n-fzg1.spec.json}"
BASE_PAYLOAD="${BASE_PAYLOAD:-${UPSTREAM_REPO}/app-src/app/src/main/assets/cve-2026-43499-app.so}"
PAYLOAD_PATH="${PAYLOAD_PATH:-${REPO_ROOT}/app/src/main/assets/cve-2026-43499-app.so}"
...
HELPER_NEXT_PATH="${HELPER_NEXT_PATH:-${REPO_ROOT}/app/src/main/jniLibs/arm64-v8a/libcve43499root_next.so}"
...
Default:
  validates the connected SM-S918N, patches the F731U base payload into the
  dm3q/S918NKSS8FZG1 payload (or reuses the bundled S918N payload if no base
  is available), prepares local app assets, and prints the manual ADB command.
...
Useful env overrides:
  ...
  SPEC=/path/to/f731u-to-dm3q-s918n-fzg1.spec.json
  PAYLOAD_PATH=/path/to/bundled-s918n-payload.so
  KSUD_PATH=/path/to/ksud-f731u-kdp
  HELPER_PATH=/path/to/libcve43499root.so
  HELPER_NEXT_PATH=/path/to/libcve43499root_next.so
...
check_device() {
  ...
  if [ -n "$TARGET_KERNEL_VERSION" ]; then
    ...
    info "kernel-version check skipped (TARGET_KERNEL_VERSION empty); device says: ${kernel_version}"
  fi
...
patch_payload() {
  if [[ ! -f "$BASE_PAYLOAD" ]]; then
    info "base payload not found: ${BASE_PAYLOAD}"
    info "skipping binary patch — will use the bundled S918N payload as-is"
    return 0
  fi
  require_cmd python3
  assert_file "$PATCHER"
...
prepare_app_assets() {
  local src_payload="$PAYLOAD_PATH"
  if [[ -f "$PATCHED_PAYLOAD" ]]; then src_payload="$PATCHED_PAYLOAD"; fi
  assert_file "$src_payload"
  assert_file "$KSUD_PATH"
  assert_file "$HELPER_PATH"
  assert_file "$HELPER_NEXT_PATH"
  assert_md5 "$src_payload" "$EXPECTED_PAYLOAD_MD5" "payload"
  assert_md5 "$KSUD_PATH" "$EXPECTED_KSUD_MD5" "KernelSU daemon"
  assert_md5 "$HELPER_PATH" "$EXPECTED_HELPER_MD5" "root helper (regular)"
  assert_md5 "$HELPER_NEXT_PATH" "$EXPECTED_HELPER_NEXT_MD5" "root helper (next)"
  ...
  cp "$src_payload" "${asset_root}/cve-2026-43499-app.so"
  cp "$KSUD_PATH" "${asset_root}/ksud-f731u-kdp"
  cp "$HELPER_PATH" "${jni_lib_dir}/libcve43499root.so"
  cp "$HELPER_NEXT_PATH" "${jni_lib_dir}/libcve43499root_next.so"

  # Both S918N profiles reuse the device-tested ksud-f731u-kdp (generic
  # android13-5.15 KMI) exactly like upstream reuses one daemon per KMI.
  cat > "${asset_root}/targets-v3.json" <<JSON
{
  "schemaVersion": 3,
  "payloads": [
    {
      "payloadId": "dm3q-S918NKSS8FZG1",
      "displayName": "Galaxy S23 Ultra SM-S918N | FZG1 Patched",
      "models": ["${TARGET_MODEL}"],
      "kernelVersions": ["5.15.189"],
      "exploit": { "url": "asset://cve-2026-43499-app.so", "size": ${EXPECTED_PAYLOAD_SIZE} },
      "kernelsu": { "url": "asset://ksud-f731u-kdp", "size": ${EXPECTED_KSUD_SIZE} }
    },
    {
      "payloadId": "dm3q-S918NKSS8FZG1-ksunext",
      "displayName": "Galaxy S23 Ultra SM-S918N | FZG1 | KernelSU Next",
      "models": ["${TARGET_MODEL}"],
      "kernelVersions": ["5.15.189"],
      "exploit": { "url": "asset://cve-2026-43499-app.so", "size": ${EXPECTED_PAYLOAD_SIZE} },
      "kernelsu": { "url": "asset://ksud-f731u-kdp", "size": ${EXPECTED_KSUD_SIZE} }
...
JSON
  ...
  info "prepared native helpers under ${jni_lib_dir}"
}
...
stage_adb_files() {
  ...
  adb push "$PAYLOAD_PATH" "${ADB_STAGE_DIR}/cve-2026-43499-app.so"
